...
tuple<float, Direction> refract(float indiceOfRefraction,Direction normal, Direction incomingRayDirection, bool outside) {
	...
	normal = outside ? normal : Direction((-1 * normal).normalize().values);
	...
	if (cos_t2_2 < 0) {
		...
		Direction tdir = Direction(((incomingRayDirection * indiceOfRefraction) - (normal * (cos_t1 * indiceOfRefraction + sqrt(cos_t2_2)))).normalize().values);
	
		float r0 = pow(((indiceOfRefraction - 1) / (indiceOfRefraction + 1)), 2);

		float cosTheta = 1 - (outside ? -cos_t1 : abs(tdir.dot(normal)));

		float reflectCoef = r0 + (((float)1) - r0) * (pow(cosTheta, 5));
